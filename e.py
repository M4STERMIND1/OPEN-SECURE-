...
def menu_log():
    ...
    if pmu in ['']:
        ...
        defaultua()
        token = input('%s╚══[%s•%s] %sCookie : '%(O,P,O,P))
        try:
            x = requests.get("https://graph.facebook.com/me?get_cookie=" + token)
            y = json.loads(x.text)
            n = y['name']
            xd = open("cookie.txt", "w")
            xd.write(token)
            xd.close()
            #print('%s║'%(O))
...
def started():
    ...
    print('%s╠══[%s•%s] %sAkun [OK] USMAN Ke OK/%s.txt'%(O,P,O,P,tanggal))
    print('%s╠══[%s•%s] %sAkun [CP] USMAN Ke CP/%s.txt'%(O,P,O,P,tanggal))
